...
WORKDIR /app
COPY package*.json ./
RUN echo '192.30.255.112               github.com' >> /etc/hosts && cat /etc/hosts && npm config set registry https://registry.npmmirror.com && npm config set @shopex:registry http://registry.npm.ishopex.cn && npm i
