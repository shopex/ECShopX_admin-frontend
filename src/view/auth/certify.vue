<template>
    <section class="section section-white content-padded" onload="init()">
      <el-col v-if="certify_status" :span="24" class="group-label certify-status">
        <span class="label-title">审核状态：{{certify_status}}</span>
      </el-col>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="营业执照" required>
              <upload-img @changeImg="changeBusinessImg" :list="form.business_license"></upload-img>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <el-form-item label="企业微信二维码" required>
              <upload-img @changeImg="changeQRImg" :list="form.wechat_qrcode"></upload-img>
            </el-form-item>
          </el-col>
          <el-col :span="8">
          ...
          <el-col :span="24" class="group-label">
            ...
            <span class="label-note"><i class="el-icon-warning icon-wr" />配置线下对公支付收欢很行账号信息，用户平台转账使用确保填写正确</span>
          </el-col>
  ...
  methods: {
    async getList() {
      ...
      if(res.supplier_info && res.supplier_info.id) {
        const { supplier_name = '', contact  = '', mobile = '', business_license = '', wechat_qrcode = '', service_tel = '', bank_account = '', bank_name = '', is_check } = res.supplier_info
        this.$nextTick(() => {
          this.form = {
            ...this.form,
            supplier_name,
    ...
    async createSupplier() {
      const { supplier_name, contact, mobile, business_license, wechat_qrcode, service_tel, bank_name, bank_account } = this.form
      const params = {
        supplier_name,
        contact, 
        mobile, 
