  ...
  <div class="zyk_bspay_account_detail">
    ...
    <el-descriptions :column="2" title="支付信息">
      ...
      <el-descriptions-item label="支付方式">
        ...
        <span v-if="list.pay_channel == 'alipay_wap'">支付宝H5支付</span>
      </el-descriptions-item>
      <el-descriptions-item label="支付对象ID">
        {{ list.username }}
      </el-descriptions-item>
    </el-descriptions>
          ...
          <template v-if="index == 0">
            <div class="key">
              ...
              >
                ...
                <div v-if="$store.getters.login_type == 'merchant'" slot="content">
                  分账金额 = (订单金额-手续费) * 商户分账占比<br>若计算金额存在三位及以上的小数，则四舍五入精确至两位小数
                </div>
                <div v-else slot="content">
                  分账金额 = (订单金额-手续费) * 总部分账占比<br>若计算金额存在三位及以上的小数，则四舍五入精确至两位小数
                </div>
              </el-tooltip>
              分账金额：
            </div>
    ...
    >
      <el-descriptions-item label="手续费扣费方式">
        <span v-if="list.bspay_fee_mode == '1'">外扣</span>
        <span v-else-if="list.bspay_fee_mode == '2'">内扣</span>
      </el-descriptions-item>
      <el-descriptions-item label="手续费">
        {{ (list.bspay_fee / 100) | formatNumMoney }}
      </el-descriptions-item>
    </el-descriptions>
    <el-descriptions v-if="list.refund_list.length > 0" :column="2" title="退款信息">
      <template v-for="item in list.refund_list">
        ...
        <el-descriptions-item :key="item.created" label="申请时间">
