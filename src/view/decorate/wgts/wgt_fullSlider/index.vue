...
<template>
  ...
  >
    <div
      v-if="refresh"
      class="wgt-bd"
      :class="{ 'spaced': value.spaced }"
      :style="setCarouselHeight"
    >
      <!-- 固定位置的叠层 -->
      <el-carousel
    ...
    <div
      ...
      :style="`bottom: ${value.dotbottom}px;color: ${value.indicatorColor};font-size: ${value.indicatorFontSize}px`"
    >
      <div>
        <span :style="`font-size: ${value.indicatorFontSize + 2}px`">{{
          value.data.length < 10 ? '0' + (currentIndex + 1) : currentIndex + 1
        }}</span>
        <span> / </span>
  ...
  methods: {
    ...
    setBgColor(item, index) {
      if (item.isCustomSlider) {
        ...
        return {
          background:
            this.currentIndex >= index ? this.value.pilot.dot_cover : this.value.pilot.dot_noCover
        }
      }
    },
