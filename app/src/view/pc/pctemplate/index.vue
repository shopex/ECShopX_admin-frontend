<template>
  <div class="website-template">
    <div v-if="$route.path.indexOf('editor') === -1">
      ...
      <el-row>
        <el-tabs type="border-card" v-model="page_type" @tab-click="handleTabClick">
          <el-tab-pane v-for="(item, index) of tabList" :key="index" :label="item.label" :disabled="loading"
            :name="item.name"></el-tab-pane>
          <el-table stripe style="width: 100%" v-loading="loading" :data="templateList">
            <el-table-column prop="theme_pc_template_id" label="页面id"></el-table-column>
            <el-table-column prop="template_title" label="页面名称"></el-table-column>
            <el-table-column prop="template_description" label="页面描述"></el-table-column>
            ...
            <el-table-column label="操作" width="250px">
              <template slot-scope="scope">
                <div>
                  ...
                  <el-button type="primary" round size="mini" @click="handleRenovation(scope.row)">页面装修</el-button>
                </div>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-dialog title="添加/编辑页面" :visible.sync="dialogVisible" width="500px" :before-close="handleDialogClose">
        <el-form :model="formData" :rules="formRule" ref="myForm" label-width="80px">
          <el-form-item label="页面名称" prop="template_title">
            <el-input clearable v-model="formData.template_title" placeholder="请输入页面名称"></el-input>
          </el-form-item>
          <el-form-item label="页面描述" prop="template_description">
            <el-input clearable v-model="formData.template_description" placeholder="请输入页面描述"></el-input>
          </el-form-item>
          <el-form-item label="页面类型" prop="page_type">
            <el-select style="width: 100%" clearable v-model="formData.page_type">
              <el-option v-for="(item, index) of tabList" :key="index" :value="item.name" :label="item.label">
              </el-option>
            </el-select>
          </el-form-item>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="submitForm('myForm')" :loading="loading">确认保存</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
          ...
          if (valid) {
            ...
            try {
              if (theme_pc_template_id === '') {
                const {
                  data
                } = await addPageTemplate(formData)
