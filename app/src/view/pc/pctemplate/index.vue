    ...
    <div v-if="$route.path.indexOf('editor') === -1">
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="submitForm('myForm')" :loading="loading"
            >确认保存</el-button
          >
        </span>
      </el-dialog>
...
export default {
  data() {
    return {
      ...
      formRule: {
        template_title: [{ required: true, message: '请输入页面名称', trigger: 'blur' }],
        template_description: [{ required: true, message: '请输入页面描述', trigger: 'blur' }],
        page_type: [{ required: true, message: '请输入页面类型', trigger: 'change' }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    handleRenovation({ theme_pc_template_id }) {
      ...
      this.$router.push({ path: '/wxapp/pcmall/design?id=' + theme_pc_template_id })
    },
