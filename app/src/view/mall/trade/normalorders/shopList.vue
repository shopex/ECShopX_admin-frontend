<template>
  <div class="">
    <div v-if="$route.path.indexOf('detail') === -1">
      ...
      <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
        ...
        <el-table :data="list" style="width: 100%" :height="wheight-190" v-loading="loading" element-loading-text="数据加载中">
          ...
          <el-table-column prop="order_status" label="订单状态">
            <template slot-scope="scope">
              <!-- 订单状态 -->
              <el-tag v-if="scope.row.order_status == 'PAYED'" type="success" size="mini">已支付</el-tag>
              <el-tag v-else-if="scope.row.order_status == 'NOTPAY'" size="mini">未支付</el-tag>
              <el-tag v-else-if="scope.row.order_status_des == 'CANCEL'" type="danger" size="mini">已取消</el-tag>
              <el-tag v-else-if="scope.row.order_status_des == 'CLOSED'" type="info" size="mini">{{scope.row.order_status_msg}}</el-tag>
              <el-tag v-else-if="scope.row.order_status == 'WAIT_BUYER_CONFIRM'" type="danger" size="mini">待收货</el-tag>
              <el-tag v-else-if="scope.row.order_status == 'REFUND_SUCCESS'" type="warning" size="mini"> 已退款</el-tag>
              <el-tag v-else-if="scope.row.order_status == 'WAIT_GROUPS_SUCCESS'" type="warning" size="mini"> 等待成团</el-tag>
              <el-tag v-else-if="scope.row.order_status == 'DONE'" type="success" size="mini">已完成</el-tag>
              <template v-if="scope.row.order_status != 'CANCEL' && scope.row.order_status != 'REFUND_SUCCESS' && scope.row.order_status != 'WAIT_GROUPS_SUCCESS'">
                <!-- 发货状态 -->
                <el-tag v-if="scope.row.cancel_status == 'WAIT_PROCESS'" type="warning" size="mini"> 取消待退款</el-tag>
                <el-tag v-else-if="scope.row.delivery_status == 'PARTAIL'" type="danger" size="mini">部分发货</el-tag>
                <el-tag v-else-if="scope.row.ziti_status == 'PENDING'" type="danger" size="mini">待自提</el-tag>
                <el-tag v-else-if="scope.row.delivery_status =='PENDING'"  size="mini">待发货</el-tag>
              </template>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" fixed="right">
            <template slot-scope="scope">
              <router-link :to="{ path: matchHidePage('detail'), query: { orderId: scope.row.order_id, resource: $route.path }}" style="margin-right: 5px">详情</router-link>
              <el-button type="text" v-if="scope.row.cancel_status=='NO_APPLY_CANCEL' && (scope.row.order_status == 'NOTPAY' || scope.row.order_status == 'PAYED')" @click="cancelOrderAction(scope.row.order_id)" style="margin-right: 5px">取消订单</el-button>
              <el-button type="text" v-if="scope.row.receipt_type=='logistics' && scope.row.order_status == 'PAYED' && scope.row.delivery_status != 'DONE'" @click="deliveryAction(scope.row)" style="margin-right: 5px">发货</el-button>
              <el-button type="text" v-if="scope.row.receipt_type == 'ziti' && scope.row.ziti_status == 'PENDING' && scope.row.order_status == 'PAYED'" @click="writeoffOrderAction(scope.row.order_id)" style="margin-right: 5px">核销</el-button>
              <el-button type="text" v-if="scope.row.cancel_status=='WAIT_PROCESS' && scope.row.order_status == 'PAYED'" @click="confirmCancelOrderAction(scope.row.order_id)" style="margin-right: 5px">退款</el-button>
              <el-button type="text" @click="clickShowRemark(scope.row,'normalList2')">备注</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <!-- 发货-开始 -->
      <el-dialog :title="deliveryTitle" :visible.sync="deliveryVisible"  :before-close="handleCancel" width="57%">
        <template>
  ...
  methods: {
    ...
    deliveryAction(data) {
      ...
      getOrderDetail(order_id).then((response) => {
        ...
        if (
          this.deliveryData &&
          this.deliveryData.orderInfo.delivery_status == "PARTAIL"
        ) {
    ...
    submitWriteoffOrderConfirmAction() {
      ...
      doWriteoff(this.writeoffOrderForm.order_id, this.writeoffOrderForm).then(
        (response) => {
    ...
    submitCancelConfirmAction() {
      // 提交取消订单审核结果
      cancelConfirm(this.cancelForm.order_id, this.cancelForm).then(
        (response) => {
      ...
      if (this.deliveryForm.delivery_type == "sep") {
        if (this.selectItemType == "old") {
          this.deliveryForm.sepInfo = JSON.stringify(
            this.deliveryData.orderInfo.items
          );
          this.deliveryForm.delivery_corp = "";
          this.deliveryForm.delivery_code = "";
        } else {
          this.deliveryForm.sepInfo = JSON.stringify(
            JSON.parse(
              JSON.stringify(this.deliveryData.orderInfo.items)
            ).filter((item) => item.delivery_num && item.delivery_num != "")
          );
        }
      } else {
    ...
    submitCancelOrderConfirmAction() {
      ...
      if (
        this.cancelOrderForm.cancel_reason == 12 &&
        this.cancelOrderForm.other_reason == ""
      ) {
