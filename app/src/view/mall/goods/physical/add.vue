...
<template>
  <div class="section-page">
    ...
    <template v-if="isEditor || selectedMainCategory.length > 0">
      ...
      <!-- 商品参数 -->
      <CmGoodsParams v-if="paramsData.length > 0" v-model="paramsData" :pointAccess="baseData.point_access" />

      <!-- 商品规格 -->
      <CmSkuForm v-model="skuData" :isEditor="isEditor" :isPackageItems="baseData.isPackageItems" :pointAccess="baseData.point_acces" @specOnChange="specOnChange"/>
        ...
        <div class="form-block-body">
          ...
          <div class="richText-block" v-if="mode === 'richText'">
            <div class="ricktext-con">
              <vue-html5-editor :content="intro.toString()" ref="editor" @change="(e) => { this.intro = e }" :height="360" style="width: 100%;"></vue-html5-editor>
            </div>
          ...
          <div class="component-block" v-else>
            <richTextEditor :data="content" :control="['film','slider','heading','writing']" @change="(data) => { content = data }"/>
          </div>
      ...
      <div class="section-footer">
        ...
        <el-button type="primary" @click="handleSave" :loading="submitLoading">{{submitLoading ? '提交中' : '保存'}}</el-button>
      </div>
    </template>
  </div>
</template>
    ...
    methods: {
      ...
      getSkuItems() {
        ...
        this.skuData.specItems = _specItmes.map(item => {
          ...
          const temp = {
            ...
            price: cacheItems[key] ? cacheItems[key].price / 100 : '',
            cost_price: cacheItems[key] ? cacheItems[key].cost_price  / 100: '',
            market_price: cacheItems[key] ? cacheItems[key].market_price / 100: '',
      ...
      getSpecName(keys) {
        ...
        keys.forEach((key, index) => {
          const fd = this.skuData.itemSpecList[index].attribute_values.list.find(item => item.attribute_value_id == key)
          if(fd) {
      ...
      async handleSave() {
        ...
        const { item_id, itemName, itemCategory, sort, taxRate, 
        brief, templatesId, brandId, itemUnit, regionsId, isProfit, 
