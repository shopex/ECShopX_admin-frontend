<template>
  <div>
    <div>
      <el-row :gutter="20">
        <el-col>
          <shop-select distributors  @update="storeSearch" :shopIdDefault="params.distributor_id"></shop-select> <!--distributors wxshops 需要哪个api传哪个-->
          ...
          <el-input class="input-m" placeholder="请输入商品货号" v-model="params.item_bn">
            <el-button slot="append" icon="el-icon-search" @click="goodsNumberSearch"></el-button>
          </el-input>
          <el-input class="input-m" placeholder="请输入商品条码" v-model="params.barcode">
            <el-button slot="append" icon="el-icon-search" @click="goodsNumberSearch"></el-button>
          </el-input>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col>
          <el-button-group>
            <el-button type="primary" @click="handleBatchOnline(true)" icon="el-icon-upload">批量上架</el-button>
            <el-button type="primary" @click="handleBatchOnline(false)" icon="el-icon-download">批量下架</el-button>
            <el-button type="primary" @click="handleBatchIsTotalStore(true)">开启总部发货</el-button>
            <el-button type="primary" @click="handleBatchIsTotalStore(false)">关闭总部发货</el-button>
      ...
      <el-tab-pane v-for="(item,index) in tabList" :key="index" :label="item.name" :name="item.activeName">
        <el-table :data="list" style="width: 100%" :height="wheight-240" v-loading="loading" ref="multipleItemsTable" @selection-change="handleItemsSelectionChange">
          ...
          <el-table-column label="上下架操作">
            <template slot-scope="scope">
              <el-switch v-model="scope.row.goods_can_sale"  @change="switchStatusChange(scope.$index, scope.row, true)"></el-switch>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="is_total_store" label="总部发货" width="80">
            <template slot-scope="scope">
              <el-switch :disabled="scope.row.distributor_id==0" v-model="scope.row.is_total_store"  active-color="#13ce66"  inactive-color="#ff4949" @change="switchChange(scope.$index, scope.row, 'true')"></el-switch>
            </template>
          </el-table-column>
    ...
    <sideBar :visible.sync="show_sideBar" title="编辑店铺sku配置" width="60">
      <slot>
        ...
        <el-table :data="itemSku.list" :height="wheight-240" v-loading="loading">
          <el-table-column prop="item_bn" label="SKU" width="140"></el-table-column>
          <el-table-column prop="item_spec_desc" label="规格"></el-table-column>
          <el-table-column prop="store" label="库存" width="120">
            <template slot-scope="scope" >
              <el-input v-if="editStore" v-model="scope.row.store" @change="editItemsStore(scope.$index, scope.row)"><i slot="suffix" class="el-input__icon el-icon-edit"></i></el-input>
              <span v-else>{{scope.row.store}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="price" label="商品价格" width="120">
            <template slot-scope="scope">
              <el-input v-if="editStore" v-model="scope.row.price" @change="editItemsPrice(scope.$index, scope.row)"><i slot="prefix" class="el-input__icon el-icon-edit"></i><i slot="suffix" class="el-input__icon">元</i></el-input>
          ...
          <el-table-column label="上下架操作" width="180">
            <template slot-scope="scope">
              <el-switch  v-model="scope.row.is_can_sale"  active-text="已上架"  inactive-text="已下架"  @change="switchStatusChange(scope.$index, scope.row, false)"></el-switch>
            </template>
          </el-table-column>
        </el-table>
        <div class="content-padded content-center">
...
<script>
  ...
  import { getDistributorItems, exportDistributorItems, delDistributorItems, updateDistributorItem, getDistributorInfo} from '../../../api/marketing'
    ...
    methods: {
      ...
      switchChange(index, row, isDefault) {
        if (row.is_total_store === true) {
          let form = {"distributor_id": row.distributor_id, "item_id": row.item_id, "is_total_store": row.is_total_store, "is_default": isDefault}
          this.updateDistributorItem(form)
            ...
            //   return;
            // }
            let form = {"distributor_id": row.distributor_id, "item_id": row.item_id, "is_total_store": row.is_total_store, "store": row.distributor_store < 0 ? row.item_store : row.distributor_store, "is_default": isDefault}
            this.updateDistributorItem(form)
        }
      },
      //变更店铺商品库存
      editItemsStore(index, row) {
        ...
        //   return;
        // }
        let form = {"distributor_id": row.distributor_id, "item_id": row.item_id, "is_total_store": row.is_total_store, "store": row.store}
        this.updateDistributorItem(form)
      },
      //变更店铺商品价格
      ...
      handleBatchOnline(goods_can_sale) {
        ...
        var form = {"distributor_id": this.params.distributor_id, "goods_id": JSON.stringify(goodsIds), "is_can_sale": goods_can_sale}
        this.updateDistributorItem(form)
      },
      handleSkuEdit(row) {
      ...
      handleItemsSelectionChange (val) { // 商品选择
      ...
      handleBatchDownload: function() {
        ...
        that.itemsChecked.forEach(item => {
          ...
          const promise = getWxaCodeImg(url).then(result_file => { // 下载文件, 并存成ArrayBuffer对象
      ...
      switchStatusChange(index, row, isDefault) {
        let form = {}
        if (isDefault) {
          form = {"distributor_id": this.params.distributor_id, "goods_id": row.goods_id, "is_can_sale": row.goods_can_sale}
        } else {
          form = {"distributor_id": this.params.distributor_id, "item_id": row.item_id, "is_can_sale": row.is_can_sale}
        }
        this.updateDistributorItem(form)
      },
      goodsNumberSearch () {
      ...
      handleBatchIsTotalStore(is_total_store) {
        ...
        var form = {"distributor_id": this.params.distributor_id, "goods_id": JSON.stringify(goodsIds), "is_total_store": is_total_store}
        this.updateDistributorItem(form)
      },
    },
    mounted () {
